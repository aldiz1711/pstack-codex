...
**pstack is my answer.** these are the same skills i use everyday to ship high quality code at Cursor, now configured for Codex. this turns codex into a real engineering team. the goal is not to maximize loc, in fact it's the opposite. pstack helps you write less, but higher quality code.
...
**codex gives you a choice of models.** every frontier model has its strengths and weaknesses. use any available model with pstack. in fact, many of my skills use multi-model workflows to take advantage of each model's unique strengths.

fork it. improve it. make it yours. PRs are welcome! 

## install

Install this repository as the `pstack-codex` plugin in your personal Codex marketplace, then run `codex plugin add pstack-codex@personal`. Open a new Codex task to load its skills.
...
that's it. the other skills are situational; the mode skill uses them for you as needed. out of the box the mode splits work by model strength: code delegates (feature, refactoring, bug fix, perf, hillclimb) use a fast Codex model, while the hardest changes, prose, and judgment use the strongest available one. the default review panel spans four available Codex models. [`$setup-pstack`](./skills/setup-pstack/SKILL.md) changes any of it.
...
[`$poteto-mode`](./skills/poteto-mode/SKILL.md) works with Codex's native `/goal` mode for a durable objective and scheduled follow-ups for later audit ticks. you can make Codex work for many hours without sacrificing rigor.
...
| [`$control-cli`](./skills/control-cli/SKILL.md) | drive and verify a CLI or TUI through a local terminal harness. |
| [`$control-ui`](./skills/control-ui/SKILL.md) | drive and verify a browser or Electron UI through a local browser harness. |
| [`$deslop`](./skills/deslop/SKILL.md) | clean code slop from the branch diff before commit. |
...
pstack also ships a subagent that runs my style end to end. spawn it from a parent agent via [Codex agent `poteto-agent`](./.codex/agents/poteto-agent.toml). it reads `poteto-mode` in full, including its inline principles index, before doing any work. substituting the general-purpose agent skips that read and drifts.

[`$poteto-mode`](./skills/poteto-mode/SKILL.md) and [Codex agent `poteto-agent`](./.codex/agents/poteto-agent.toml) use the same instructions.

pstack also ships [Comment Sicko](./.codex/agents/comment-sicko.toml), a comment reviewer available as Codex agent `comment-sicko`. usually invoke it through [`$no-comments`](./skills/no-comments/SKILL.md), not directly.
...
## control and cleanup skills

pstack includes the three skills it uses from Cursor Team Kit: [`$control-cli`](./skills/control-cli/SKILL.md) for CLIs and TUIs, [`$control-ui`](./skills/control-ui/SKILL.md) for browser and Electron UIs, and [`$deslop`](./skills/deslop/SKILL.md) for code cleanup. They are copied without changes from [cursor/plugins](https://github.com/cursor/plugins/tree/main/cursor-team-kit/skills) under the [Cursor Team Kit MIT license](./licenses/cursor-team-kit-LICENSE). The skills use tools available in the project or Codex environment.

Codex's `$skill-creator` authors skills. Inside `poteto-mode`, the [babysit playbook](./skills/poteto-mode/playbooks/babysit.md) handles PR-status requests.

## why are there no planning skills?

codex has a plan mode which works with pstack. but personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`$poteto-mode`](./skills/poteto-mode/SKILL.md) covers it, but it's not a default. 
...
models are configurable too. type [`$setup-pstack`](./skills/setup-pstack/SKILL.md). it detects the models you have access to and writes a small role map at `~/.codex/pstack-models.md`, defaulting to the original unlimited budget. the bundled hook loads it for new Codex tasks and subagents after you trust the hook in Codex. every skill falls back to sensible defaults when the file is absent, so you override only what you want.
