...
keep working until done. if the task needs later wakeups, schedule follow-ups. if you're truly stuck after a few hours, stop and write up why.
```
...
- Codex can continue within the current task. For later wakeups, use a scheduled follow-up tied to the same finish condition. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) re-checks it on each run.
...
**Pitfall:** a duration is not a finish condition. "work on this for 4 hours" gives the agent nothing to check, and you'll wake up to four hours of motion instead of a result. Give the task a predicate that can pass or fail.
