...
Codex task history is available through the current conversation context and, where exposed, the task-list and task-reader tools. Read only tasks in the active workspace unless the user asks for a broader scope.
...
3. Fan out across your task history when there is enough history and Codex exposes task-list and task-reader tools. Spawn parallel subagents on a fast model, each taking a slice of the active workspace's tasks. Order candidates by recency, search the topic first, read only relevant task turns, and skip the current task plus obvious noise (subagent, eval, and test tasks). Each returns the same schema, one block per task: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the task ID. For one or two tasks, skip the fan-out and search directly. The main thread gets only their findings. If task history is unavailable, use the current context and live state, and state the gap.
