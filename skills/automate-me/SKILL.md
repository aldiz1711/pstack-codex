...
Look recursively for `.agents/skills/**/*-mode/SKILL.md` and `~/.codex/skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.agents/skills/<handle>/`), not only at the top level. If one exists, confirm intent with Codex user-input tool (unless they already said "update my skill" or similar):
...
Locate recent Codex tasks in the active workspace through the task-list and task-reader tools if available. Do not read unrelated projects. If task history is unavailable, use the current conversation and ask for a relevant task link only when needed.

Survey recent agent conversations within that scope for recurring patterns. Run multiple parallel subagents across slices of history (e.g. last 2-4 weeks, split into 3 slices so each has enough material). Each slice mining subagent reads the in-scope Codex tasks the parent identifies, looks for the signals below, and returns a short structured list of patterns it saw with evidence pointers. Default signals worth hunting:
...
Mining misses intent that hasn't come up yet. Use an available Codex user-input tool for short preference questions.

Shape: one or two questions with the number of choices the available tool supports. Start broad ("Which areas matter most?"), then follow up on selected areas with specific options. After the structured rounds, one free-form chat question catches anything the options missed.
